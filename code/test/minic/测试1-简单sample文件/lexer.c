...
static struct
{
	const char* str;
	const char* value;
} reservedWords[24] = {{"+","PLUS"},{"(","LPAN"},{")","RPAN"},{"*","MULTIPLY"},{"[","LMM"},{"]","RMM"},{"!=","NE"},{"%","MOD"},{"*",""},{",","DOU"},{"-","MINUS"},{"/","DIVIDE"},{":=","ASSIGN"},{";","SEMI"},{"<","LT"},{"<=","LTEQ"},{"<>","NE"},{"=","ASSIGN"},{"==","EQ"},{">","RT"},{">=","RTEQ"},{"^","CARET"},{"{","LBM"},{"}","RBM"} };
//lex编码的映射
static struct{
	const char* op;
	const int code;
...
int main(int argc, char* argv[]) {
	FILE* input_file = fopen("G:/04code/10Qt/byyl-combined6/test/minic/测试1-简单sample文件/sample.minic", "r");
	if (input_file == NULL) {
		printf("Failed to open input file");
		return 1;
	}
	FILE* output_file = fopen("G:/04code/10Qt/byyl-combined6/test/minic/测试1-简单sample文件/sample.lex", "w");
	if (output_file == NULL) {
